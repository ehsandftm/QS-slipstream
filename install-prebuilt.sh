...
# QS + Slipstream Local (Client) Installer - Pre-built Binary Version
# Usage: BINARY_PATH=/ubuntu/qs-slip/slipstream-client-rust bash install-prebuilt.sh
...
if [ -z "$BINARY_PATH" ]; then
  echo "[!] ERROR: BINARY_PATH not set!"
  echo ""
  echo "Usage:"
  echo "  BINARY_PATH=/ubuntu/qs-slip/slipstream-client-rust bash install-prebuilt.sh"
  echo ""
  echo "Or download the binary first:"
  echo "  mkdir -p /ubuntu/qs-slip"
  echo "  # Upload slipstream-client-rust binary to /ubuntu/qs-slip/ via Telegram/SCP"
  echo "  chmod +x /ubuntu/qs-slip/slipstream-client-rust"
  echo "  BINARY_PATH=/ubuntu/qs-slip/slipstream-client-rust bash install-prebuilt.sh"
  exit 1
fi
...
if [ ! -x "$BINARY_PATH" ]; then
  echo "[!] Binary not executable. Making it executable..."
  chmod +x "$BINARY_PATH"
fi
...
echo "   Example: qs.gtgp.space or q.firmware-update-service.com"
...
# ─────────────────────────────────────────────
echo "[+] Installing slipstream-client-rust binary from $BINARY_PATH..."
cp "$BINARY_PATH" "$RUST_CLIENT"
chmod +x "$RUST_CLIENT"
